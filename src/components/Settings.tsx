...
import type { MessageDisplayMode } from "@/hooks/useMessageDisplayMode";

/**
 * 支持的消息显示模式列表，确保 UI 与存储逻辑一致
 */
const MESSAGE_DISPLAY_MODES: MessageDisplayMode[] = ["cmd", "enhanced", "hybrid", "terminal", "both"];
...
export const Settings: React.FC<SettingsProps> = ({ onBack, className }) => {
  ...
  const [messageDisplayMode, setMessageDisplayMode] = useState<MessageDisplayMode>('cmd');
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      setCustomMultiplierInput(fontScaleManager.getCustomMultiplier().toString());
          ...
          // 加载消息显示模式，优先使用配置，其次读取本地缓存，最后回退到混合模式
    const savedDisplayMode = (loadedSettings.messageDisplayMode as string | undefined) ?? localStorage.getItem('messageDisplayMode') ?? 'hybrid';
    
    // 处理历史模式映射
    let normalizedMode = savedDisplayMode;
    if (savedDisplayMode === 'tool_results_only' || savedDisplayMode === 'tool_calls_only') {
      normalizedMode = 'hybrid'; // 将简洁模式映射为混合模式
    }
    
    if (typeof normalizedMode === 'string' && MESSAGE_DISPLAY_MODES.includes(normalizedMode as MessageDisplayMode)) {
      setMessageDisplayMode(normalizedMode as MessageDisplayMode);
    } else {
      setMessageDisplayMode('hybrid');
    }
    } catch (err) {
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-3">
                          {[
                            { value: 'hybrid', label: t.settings.showHybridMode, desc: t.settings.showHybridModeDesc, disabled: false },
                            { value: 'cmd', label: t.settings.showCmdMode + ' (正在开发中)', desc: t.settings.showCmdModeDesc, disabled: true },
                            { value: 'enhanced', label: t.settings.showEnhancedMode + ' (正在开发中)', desc: t.settings.showEnhancedModeDesc, disabled: true },
                            { value: 'terminal', label: t.settings.showTerminalMode + ' (正在开发中)', desc: t.settings.showTerminalModeDesc, disabled: true },
                            { value: 'both', label: t.settings.showBoth + ' (正在开发中)', desc: t.settings.showBothDesc, disabled: true },
                          ].map((option) => (
                            <div key={option.value} className={`flex items-start space-x-3 ${option.disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                              <input
                                type="radio"
                                ...
                                onChange={(e) => {
                                  if (!option.disabled) {
                                    setMessageDisplayMode(e.target.value as MessageDisplayMode);
                                  }
                                }}
                              ...
                              <div className="flex-1">
                                ...
                                <p className={`text-xs ${
                                  option.disabled ? 'text-gray-500' : 'text-muted-foreground'
                                }`}>
                                  {option.desc}
                                </p>
                              </div>
