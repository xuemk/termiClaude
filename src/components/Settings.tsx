...
export const Settings: React.FC<SettingsProps> = ({ onBack, className }) => {
  ...
  useEffect(() => {
    const handleEnvironmentGroupsUpdate = () => {
      logger.debug("Environment groups updated from main interface, syncing settings page...");
      reloadEnvironmentGroups();
    };

    window.addEventListener('environment-groups-updated', handleEnvironmentGroupsUpdate);
    return () => window.removeEventListener('environment-groups-updated', handleEnvironmentGroupsUpdate);
  }, [reloadEnvironmentGroups]);
  ...
  const toggleGroupEnabled = async (groupId: number, enabled: boolean) => {
    try {
      ...
      logger.info(`${enabled ? 'Enabling' : 'Disabling'} environment variable group: ${group.name}`);
      ...
      try {
        ...
        // 🔧 关键修复：启用分组时，同步更新localStorage中的模型选择
        if (enabled) {
          // 查找启用分组中的MID_1模型
          const enabledGroupVars = normalizedVars.filter(v => v.group_id === groupId && v.enabled);
          ...
          if (mid1Var && mid1Var.value.trim()) {
            ...
            logger.warn(`No MID_1 found in enabled group ${group.name}`);
          }
        } else {
          // 禁用分组时，清除localStorage中的模型选择（可选）
          logger.info("Group disabled, keeping existing model selection in localStorage");
        }
        
        logger.info(`Environment variable group '${group.name}' ${enabled ? 'enabled' : 'disabled'} successfully`);
        logger.info(`Reloaded ${updatedGroups.length} groups and ${normalizedVars.length} variables`);
      } catch (error) {
        logger.error("Failed to reload environment variables after toggle:", error);
      }
      ...
      setToast({ 
        message: `Environment group ${enabled ? 'enabled' : 'disabled'} successfully`, 
        type: "success" 
      });
    } catch (error) {
  ...
  const removeEnvVar = async (index: number) => {
    ...
    try {
      await api.saveEnvironmentVariables(updatedVars);
      setToast({ message: "Environment variable deleted", type: "success" });
    } catch (error) {
      logger.error("Failed to delete environment variable:", error);
      setToast({ message: "Failed to delete environment variable", type: "error" });
      // 如果保存失败，回滚本地状态
      setEnvVars(envVars);
    }
  };
                            ...
                            <div className="p-3 space-y-3">
                              {groupVars.length === 0 ? (
                                ...
                                groupVars.map((envVar) => {
                                  const globalIndex = envVars.findIndex(v => 
                                    v.id ? v.id === envVar.id : 
                                    (v.key === envVar.key && v.group_id === envVar.group_id)
                                  );
                                  return (
                                    <motion.div
                                      key={`env-${globalIndex}`}
