...
import { useTrackEvent, useComponentMetrics, useWorkflowTracking, useMessageDisplayMode, useHybridMode, useDisplayModePerformance } from "@/hooks";
import { HybridModeToggle, PerformanceBadge } from "./HybridModeToggle";
import { PerformanceMonitor } from "./PerformanceMonitor";
import { FallbackMechanism } from "./FallbackMechanism";
import { useStreamOptimization, createDebouncedUpdater, memoryOptimizer } from "@/lib/streamOptimization";
...
export const ClaudeCodeSession: React.FC<ClaudeCodeSessionProps> = ({
  ...
  // Get message display mode and hybrid mode management
  const { mode: messageDisplayMode } = useMessageDisplayMode();
  const hybridMode = useHybridMode();
  useDisplayModePerformance(); // Performance monitoring (used by PerformanceMonitor component)
  const streamOptimization = useStreamOptimization();

  // Create debounced updater for high-frequency message updates - reserved for future use
  useRef(
    createDebouncedUpdater((newMessages: ClaudeStreamMessage[]) => {
      setMessages(newMessages);
    }, hybridMode.isInCmdMode ? 8 : 16) // Faster updates for CMD mode
  );

  // Helper function to map display mode to performance badge mode
  const getPerformanceBadgeMode = (): 'cmd' | 'enhanced' | 'hybrid' => {
    // 直接使用消息显示模式，确保与设置一致
        ...
        if (message.type === "assistant" && message.message) {
          ...
          for (let i = index + 1; i < Math.min(index + 3, messages.length); i++) {
            ...
            if (nextMsg.type === "result" && (nextMsg.result || nextMsg.error)) {
              return false; // Skip the assistant message since we'll show the result
            }
          }
        }
        ...
        if (message.type === "result" && (message.result || message.error)) {
          ...
          for (let i = Math.max(0, index - 3); i < index; i++) {
            ...
            if (prevMsg.type === "result" && (prevMsg.result || prevMsg.error)) {
              return false; // Skip duplicate result message
            }
          }
        }
      }
  ...
  useEffect(() => {
    if (displayableMessages.length > 0) {
      // Use different scroll strategies based on mode
      const scrollBehavior = hybridMode.isInCmdMode ? "auto" : "smooth";
      const scrollDelay = hybridMode.isInCmdMode ? 0 : 100;
      ...
      requestAnimationFrame(() => {
        try {
          ...
          // 双重保障：滚动到底部，但为输入框预留一点空间（避免完全遮挡）
          setTimeout(() => {
            const container = parentRef.current;
            if (container) {
              const { scrollHeight, clientHeight } = container;
              // 只稍微向上偏移40px，避免被输入框完全遮挡，但保持原有的底部滚动行为
              const targetScrollTop = scrollHeight - clientHeight - 40;
              if (container.scrollTop < targetScrollTop) {
                container.scrollTo({
                  top: Math.max(0, scrollHeight - clientHeight - 40),
                  behavior: scrollBehavior,
                });
              }
            }
  ...
  }, [displayableMessages.length, rowVirtualizer, hybridMode.isInCmdMode]);
  ...
  useEffect(() => {
    const checkMemoryUsage = () => {
      if (memoryOptimizer.shouldOptimize()) {
        ...
        streamOptimization.cleanup();

        // If message count is very high, consider truncating
        if (messages.length > 1000) {
          setMessages(prev => prev.slice(-500)); // Keep last 500 messages
          setRawJsonlOutput(prev => prev.slice(-500));
          logger.info("Truncated messages for memory optimization");
        }
    ...
    const memoryCheckInterval = setInterval(checkMemoryUsage, 30000); // Check every 30 seconds
    return () => clearInterval(memoryCheckInterval);
  }, [messages.length, streamOptimization]);
  ...
  */
  ...
  const messagesList = (
    <div
      ref={parentRef}
      className={cn(
        "flex-1 overflow-y-auto relative pb-20",
        "virtual-container",
        hybridMode.isInCmdMode && "cmd-mode-container",
        hybridMode.isInEnhancedMode && "enhanced-mode",
          ...
          {rowVirtualizer.getVirtualItems().map((virtualItem) => {
            ...
            return (
              <motion.div
                ...
                initial={{ 
                  opacity: hybridMode.isInCmdMode ? 1 : 0, 
                  y: hybridMode.isInCmdMode ? 0 : 20 
                }}
                ...
                transition={{
                  duration: hybridMode.isInCmdMode ? 0 : 0.2,
                  ease: "easeOut"
                }}
                className={cn(
                  "absolute inset-x-4 pb-4",
                  "virtual-item",
                  hybridMode.isInCmdMode && "cmd-message-instant",
                  "stream-optimized will-change-transform"
                )}
                style={{
      ...
      {isLoading && (
        <motion.div
          ...
          className={cn(
            ...
            hybridMode.isInCmdMode && "cmd-streaming-text"
          )}
        >
          {hybridMode.isInCmdMode ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <PerformanceBadge mode={getPerformanceBadgeMode()} />
                </div>
