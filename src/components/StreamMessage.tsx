...
const StreamMessageComponent: React.FC<StreamMessageProps> = ({
  ...
  try {
    ...
    // System initialization message - completely hidden as requested
    if (message.type === "system" && message.subtype === "init") {
      // Always skip rendering system init messages
      return null;
    }
              ...
              <div className="flex-1 space-y-2 min-w-0">
                {msg.content && Array.isArray(msg.content) && msg.content.map((content: any, idx: number) => {
                  ...
                  if (content.type === "text") {
                    ...
                    return (
                      <div key={idx} className="prose prose-sm dark:prose-invert max-w-none markdown-optimized">
                        <ReactMarkdown
                            ...
                            code({ node, inline, className, children, ...props }: any) {
                              ...
                              return !inline && match ? (
                                <SyntaxHighlighter
                                  ...
                                  language={match[1]}
                                  PreTag="div"
                                  className="syntax-highlight-optimized"
                                  {...props}
                                >
                ...
                {Array.isArray(msg.content) &&
                  msg.content.map((content: unknown, idx: number) => {
                    ...
                    if (contentObj.type === "tool_result") {
                      ...
                      // Check if this is a Read tool result (contains line numbers with separator)
                      const isReadResult =
                        (content as { tool_use_id?: string }).tool_use_id &&
                        typeof contentText === "string" &&
                        /^\s*\d+(?:→|:\s*|\s+)/.test(contentText);
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 space-y-2">
                ...
                {message.result && (
                  <div className="prose prose-sm dark:prose-invert max-w-none markdown-optimized">
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm]}
                      components={{
                        code(
