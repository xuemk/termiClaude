...
const MAX_TERMINAL_JSON_LENGTH = 4000;

/**
 * 将复杂结构安全地序列化为终端可读文本
 */
const stringifyForTerminal = (value: unknown): string => {
  ...
  try {
    ...
    if (serialized && serialized.length > MAX_TERMINAL_JSON_LENGTH) {
      return `${serialized.slice(0, MAX_TERMINAL_JSON_LENGTH)}
... [截断 ${serialized.length - MAX_TERMINAL_JSON_LENGTH} 字符]`;
    }
...
/**
 * 将消息内容转换为纯文本行，尽量贴近 CLI 展示
 */
const extractContentSegments = (content?: MessageContentLike[] | unknown): string[] => {
  if (!content) {
    return [];
  }
  ...
  if (!Array.isArray(content)) {
    return [stringifyForTerminal(content)];
  }

  const segments: string[] = [];
  content.forEach((item) => {
    const entry = item as MessageContentLike;
    switch (entry.type) {
      case 'text':
        if (entry.text) {
        ...
        break;
      case 'tool_use': {
        const header = entry.name ? `→ 使用工具 ${entry.name}` : '→ 使用工具';
        segments.push(header);
        if (entry.input) {
          segments.push(stringifyForTerminal(entry.input));
        }
        break;
      }
      case 'tool_result':
      ...
      case 'command_output':
        if (entry.content) {
          segments.push(stringifyForTerminal(entry.content));
        } else if (entry.text) {
          segments.push(entry.text);
        }
        break;
      default:
        if (entry.text) {
          ...
          segments.push(stringifyForTerminal(entry));
        }
        break;
    }
  });
 ...
 * 为终端模式构造消息文本
 */
const buildTerminalMessage = (
  message: ClaudeStreamMessage,
): string | null => {
  const lines: string[] = [];
  const messageType = String((message as Record<string, unknown>).type ?? '');

  const appendWithPrefix = (prefix: string | null, segments: string[]) => {
    if (!segments.length) {
      return;
    }
    if (!prefix) {
  ...
  switch (messageType) {
    case 'system': {
      const subtype = message.subtype ? `[system:${message.subtype}]` : '[system]';
      appendWithPrefix(subtype, extractContentSegments(message.message?.content));
    ...
    case 'user':
      appendWithPrefix('>', extractContentSegments(message.message?.content));
      break;
    case 'assistant':
    case 'partial':
      appendWithPrefix(null, extractContentSegments(message.message?.content));
      break;
    case 'result':
      if (message.error) {
  ...
  if ('summary' in message && message.summary) {
    lines.push(stringifyForTerminal((message as Record<string, unknown>).summary));
  }

  if ('thinking' in message && message.thinking) {
    lines.push(stringifyForTerminal((message as Record<string, unknown>).thinking));
  }

  if ('status' in message && message.status) {
    lines.push(`[status] ${stringifyForTerminal((message as Record<string, unknown>).status)}`);
  }
...
const StreamMessageComponent: React.FC<StreamMessageProps> = ({
  ...
  // Get message display mode and hybrid mode state
  const { mode: rawMode } = useMessageDisplayMode();
  const mode: MessageDisplayMode = rawMode;
  const { isInCmdMode } = useHybridMode();

  // Extract all tool results from stream messages - MUST be called before any conditional returns
  useEffect(() => {
  ...
  // CMD/Terminal mode early return - AFTER all hooks are called
  if (mode === 'hybrid' && isInCmdMode) {
    const terminalText = buildTerminalMessage(message);
    if (!terminalText) {
      return null;
    }
    return (
      <div className={cn('cmd-terminal-block', className)}>
        <pre className="cmd-terminal-pre">{terminalText}</pre>
      </div>
    );
  }

  // Hybrid mode - switch based on user preference or session state
  const isHybridMode = mode === 'hybrid';
  const useTerminalInHybrid = isHybridMode && isInCmdMode;

  // Debug: Monitor mode changes
  if (isHybridMode) {
    console.log(`[StreamMessage] Hybrid mode - using ${isInCmdMode ? 'CMD' : 'Enhanced'} rendering`);
  }

  if (isHybridMode && useTerminalInHybrid) {
    const terminalText = buildTerminalMessage(message);
    if (!terminalText) {
      return null;
    }
    return (
      <div className={cn('cmd-terminal-block hybrid-mode', className)}>
        <pre className="cmd-terminal-pre">{terminalText}</pre>
      </div>
    );
  }
  ...
  try {
    ...
    // System initialization message - always hidden for cleaner UX
    if (message.type === "system" && message.subtype === "init") {
      // Always skip rendering system init messages to maintain clean interface
      return null;
    }
              ...
              <div className="flex-1 space-y-2 min-w-0">
                {msg.content && Array.isArray(msg.content) && msg.content.map((content: any, idx: number) => {
                  ...
                  if (content.type === "tool_use") {
                    ...
                    // Function to render the appropriate tool widget with streaming support
                    const renderToolWidget = () => {
                      // 所有模式都显示工具组件（不跳过）
                      // 避免重复的逻辑在结果渲染部分处理

                      // In CMD mode, add stream indicator for tools without results
                      const isStreaming = !toolResult && ((mode === 'hybrid' && isInCmdMode) || (mode === 'hybrid' && useTerminalInHybrid));
                      const streamingProps = isStreaming ? { isStreaming: true } : {};
                      // Task tool - for sub-agent tasks
                      if (toolName === "task" && input) {
                        renderedSomething = true;
                        return <TaskWidget description={input.description} prompt={input.prompt} result={toolResult} {...streamingProps} />;
                      }

                      // Edit tool
                      if (toolName === "edit" && input?.file_path) {
                        renderedSomething = true;
                        return <EditWidget {...input} result={toolResult} {...streamingProps} />;
                      }

                      // MultiEdit tool
                      if (toolName === "multiedit" && input?.file_path && input?.edits) {
                        renderedSomething = true;
                        return <MultiEditWidget {...input} result={toolResult} {...streamingProps} />;
                      }

                      // MCP tools (starting with mcp__)
                      if (content.name?.startsWith("mcp__")) {
                        renderedSomething = true;
                        return <MCPWidget toolName={content.name} input={input} result={toolResult} {...streamingProps} />;
                      }

                      // TodoWrite tool
                      if (toolName === "todowrite" && input?.todos) {
                        renderedSomething = true;
                        return <TodoWidget todos={input.todos} result={toolResult} {...streamingProps} />;
                      }

                      // TodoRead tool
                      if (toolName === "todoread") {
                        renderedSomething = true;
                        return <TodoReadWidget todos={input?.todos} result={toolResult} {...streamingProps} />;
                      }

                      // LS tool
                      if (toolName === "ls" && input?.path) {
                        renderedSomething = true;
                        return <LSWidget path={input.path} result={toolResult} {...streamingProps} />;
                      }

                      // Read tool
                      if (toolName === "read" && input?.file_path) {
                        renderedSomething = true;
                        return <ReadWidget filePath={input.file_path} result={toolResult} {...streamingProps} />;
                      }

                      // Glob tool
                      if (toolName === "glob" && input?.pattern) {
                        renderedSomething = true;
                        return <GlobWidget pattern={input.pattern} result={toolResult} {...streamingProps} />;
                      }

                      // Bash tool
                      if (toolName === "bash" && input?.command) {
                        renderedSomething = true;
                        return <BashWidget command={input.command} description={input.description} result={toolResult} {...streamingProps} />;
                      }

                      // Write tool
                      if (toolName === "write" && input?.file_path && input?.content) {
                        renderedSomething = true;
                        return <WriteWidget filePath={input.file_path} content={input.content} result={toolResult} {...streamingProps} />;
                      }

                      // Grep tool
                      if (toolName === "grep" && input?.pattern) {
                        renderedSomething = true;
                        return <GrepWidget pattern={input.pattern} include={input.include} path={input.path} exclude={input.exclude} result={toolResult} {...streamingProps} />;
                      }
                ...
                {Array.isArray(msg.content) &&
                  msg.content.map((content: unknown, idx: number) => {
                    ...
                    if (contentObj.type === "tool_result") {
                      ...
                      // 完整模式显示所有内容包括重复，其他模式避免重复
                      if (mode !== 'both' && hasCorrespondingWidget) {
                        return null;
                      }
