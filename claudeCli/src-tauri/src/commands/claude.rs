...
pub async fn execute_claude_code(
    ...
    env_vars.insert("CLAUDIA_MCP_SCOPE_PRIORITY".to_string(), "user,project,local".to_string());
...
/// Updates Claude settings.json with environment variables from a specific group
/// This ensures that the Claude Code CLI uses the correct environment variables
#[tauri::command]
pub async fn update_claude_settings_with_env_group(
    ...
    // 标记为内部更新（避免触发监听器）
    // Note: settings_monitor module not available in claudeCli, skipping internal update marking
    log::debug!("Performing internal settings update for environment group: {:?}", group_id);
