...
pub async fn update_claude_settings_with_env_group(
    ...
    // Intelligent merge of environment variables
    let settings_obj = settings.as_object_mut().unwrap();
    
    // Preserve existing env object and intelligently merge
    let existing_env = settings_obj.get("env")
        .and_then(|e| e.as_object())
        .cloned()
        .unwrap_or_default();

    // Create new env object with our managed variables
    let mut new_env_obj = serde_json::Map::new();

    // First, preserve non-managed external variables
    for (key, value) in &existing_env {
        if !is_managed_by_termiclaude(key, &env_vars) {
            new_env_obj.insert(key.clone(), value.clone());
            log::debug!("Preserving external variable: {} = {:?}", key, value);
        }
    }

    // 写入运行时需要的核心参数，支持多种API提供商
    let potential_runtime_keys = [
        // Anthropic API
        ...
        "ZHIPU_API_KEY", "QWEN_API_KEY", "GLM_API_KEY",
    ];
    
    for key in &potential_runtime_keys {
        if let Some(value) = env_vars.get(*key) {
            new_env_obj.insert(key.to_string(), serde_json::Value::String(value.clone()));
            log::debug!("Setting runtime variable: {} = {}", key, value);
        }
    }

    // 🔧 FIX: 确保选择的模型来自当前启用的环境变量组
    let current_selected_model = {
        // 首先尝试从当前环境变量中找到首选模型
        let preferred_from_env = find_preferred_model(&env_vars);
        ...
        if let Some(stored) = stored_model {
            // 检查存储的模型是否在当前启用的组中
            let model_exists_in_current_env = env_vars.iter()
                .any(|(key, value)| key.starts_with("MID_") && value == &stored);
            
            if model_exists_in_current_env {
                log::info!("Using validated stored model: {}", stored);
                Some(stored)
            } else {
                log::warn!("Stored model '{}' not found in current environment group, switching to preferred model from environment", stored);
                // 🔧 关键修复：同步更新内部存储的模型选择
                if let Some(preferred) = &preferred_from_env {
                    if let Err(e) = crate::commands::storage::save_app_setting(
                        app.clone(),
                        "current_selected_model".to_string(),
                        preferred.clone()
                    ).await {
                        log::error!("Failed to update stored model selection: {}", e);
                    } else {
                        log::info!("🔄 Updated stored model selection to: {}", preferred);
                    }
                }
                preferred_from_env
            }
        } else {
            log::info!("No stored model found, using preferred model from environment");
            // 🔧 新增：如果没有存储的模型，也要设置首选模型为存储值
            if let Some(preferred) = &preferred_from_env {
                if let Err(e) = crate::commands::storage::save_app_setting(
                    app.clone(),
                    "current_selected_model".to_string(),
                    ...
                    log::error!("Failed to set initial stored model selection: {}", e);
                } else {
                    log::info!("🆕 Set initial stored model selection to: {}", preferred);
                }
            }
            preferred_from_env
        }
    ...
    if let Some(model) = current_selected_model {
        new_env_obj.insert("ANTHROPIC_MODEL".to_string(), serde_json::Value::String(model.clone()));
        log::info!("🎯 Set ANTHROPIC_MODEL to: {}", model);
        
        // 🔧 关键修复：确保当前选择的模型也保存到app_settings表中
        // 这样下次启动时就能正确读取到这个模型选择
        if let Err(e) = crate::commands::storage::save_app_setting(
            app.clone(),
            "current_selected_model".to_string(),
            model.clone()
        ).await {
            log::error!("Failed to save current_selected_model to app_settings: {}", e);
        } else {
            log::info!("💾 Saved current_selected_model to app_settings: {}", model);
        }
    }

    // Update the settings with the merged env
    settings_obj.insert("env".to_string(), serde_json::Value::Object(new_env_obj));
    ...
    log::info!("Successfully updated Claude settings.json with core runtime parameters");
    Ok("Claude settings updated with core runtime parameters".to_string())
}

/// Check if a variable key is managed by TermiClaude
fn is_managed_by_termiclaude(key: &str, internal_vars: &std::collections::HashMap<String, String>) -> bool {
    // Variables managed by TermiClaude
    let managed_prefixes = [
        "MID_", "MNAME_", "MDESC_",           // Model definition variables
        "ANTHROPIC_", "OPENAI_",              // API provider variables  
        "API_BASE_URL", "API_KEY",            // Generic API variables
        "ZHIPU_", "QWEN_", "GLM_",           // Chinese AI providers
    ];
    
    // Check if key matches any managed prefix
    ...
    // Check if key is explicitly in our internal variables
    internal_vars.contains_key(key)
}

/// Find the preferred model from environment variables (prioritize MID_* over direct ANTHROPIC_MODEL)
fn find_preferred_model(env_vars: &std::collections::HashMap<String, String>) -> Option<String> {
    ...
    for i in 1..=10 {
        ...
        if let Some(model) = env_vars.get(&mid_key) {
            if !model.trim().is_empty() {
                log::info!("Found preferred model in {}: {}", mid_key, model);
...
pub async fn update_claude_settings_with_model(
    ...
    if let Err(e) = crate::commands::storage::save_app_setting(
        ...
        log::warn!("Failed to save current selected model: {}", e);
    }
...
/// Deletes a specific CLAUDE.md file by its absolute path
#[tauri::command]
pub async fn delete_claude_md_file(file_path: String) -> Result<String, String> {
    log::info!("Deleting CLAUDE.md file: {}", file_path);
    ...
    if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
        if !file_name.eq_ignore_ascii_case("CLAUDE.md") {
            return Err("Only CLAUDE.md files can be deleted through this command".to_string());
        }
    } else {
...
pub async fn execute_claude_code(
    ...
    let mut env_vars = match get_enabled_environment_variables(app.state::<AgentDb>()).await {
        Ok(vars) => {
            ...
            for (key, value) in &vars {
                if key.starts_with("ANTHROPIC_") || key.starts_with("MID_") {
                    log::info!("New session env var: {}={}", key, value);
                } else {
    ...
    if !disabled_servers.is_empty() {
        ...
        env_vars.insert("TERMICLAUDE_DISABLED_MCP_SERVERS".to_string(), disabled_list);
    }

    // Set MCP scope priority: user -> project -> local
    env_vars.insert("TERMICLAUDE_MCP_SCOPE_PRIORITY".to_string(), "user,project,local".to_string());
    log::info!("Setting MCP scope priority: user,project,local");

    // 🔧 FIX: 确保运行时使用用户选择的模型，而不是默认的 MID_1
    env_vars.insert("ANTHROPIC_MODEL".to_string(), model.clone());
    log::info!("🎯 Setting ANTHROPIC_MODEL to user selected model: {}", model);
...
pub async fn resume_claude_code(
    ...
    let mut env_vars = match get_enabled_environment_variables(app.state::<AgentDb>()).await {
        Ok(vars) => {
            log::info!("Retrieved {} environment variables for resume", vars.len());
            // Log important environment variables for debugging
            for (key, value) in &vars {
                if key.starts_with("ANTHROPIC_") || key.starts_with("MID_") {
                    log::info!("Resume env var: {}={}", key, value);
                }
            }
            vars
        },
