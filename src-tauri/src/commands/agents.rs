...
/// This ensures atomic operations and proper mutual exclusivity
#[tauri::command]
pub async fn toggle_environment_variable_group_exclusive(
    app: AppHandle,
    db: State<'_, AgentDb>, 
    group_id: i64, 
    enabled: bool
) -> Result<Vec<EnvironmentVariableGroup>, String> {
    // 分离数据库操作和异步操作，避免Send trait问题
    let groups = {
        let conn = db.0.lock().map_err(|e| e.to_string())?;
        ...
        let result = (|| -> Result<Vec<EnvironmentVariableGroup>, String> {
            if enabled {
                // If enabling this group, disable ALL other groups first (mutual exclusivity)
                conn.execute(
                    "UPDATE environment_variable_groups SET enabled = 0 WHERE id != ?1",
                    params![group_id],
                ).map_err(|e| e.to_string())?;
                
                log::info!("Disabled all other environment variable groups for mutual exclusivity");
            }
            
            // Update the target group
            conn.execute(
                "UPDATE environment_variable_groups SET enabled = ?1 WHERE id = ?2",
                params![enabled, group_id],
            ).map_err(|e| e.to_string())?;
            
            // Get the updated group info for logging
            let group_name: String = conn.query_row(
                "SELECT name FROM environment_variable_groups WHERE id = ?1",
                params![group_id],
                |row| row.get(0)
            ).map_err(|e| e.to_string())?;
            
            log::info!("Environment variable group '{}' {} successfully", 
                      group_name, if enabled { "enabled" } else { "disabled" });
            
            // Fetch all groups to return current state
            let mut stmt = conn.prepare(
                "SELECT id, name, description, enabled, sort_order, is_system, created_at, updated_at 
                 FROM environment_variable_groups ORDER BY sort_order, name"
            ).map_err(|e| e.to_string())?;
    ...
    // 现在在没有锁的情况下调用异步函数
    if let Err(e) = crate::commands::claude::update_claude_settings_with_env_group(
        app.clone(), 
        if enabled { Some(group_id) } else { None }
    ).await {
        log::error!("Failed to update Claude settings after group toggle: {}", e);
    }
    
    // 🔧 关键修复：通知前端模型选择已更新，触发主界面刷新
    if let Err(e) = app.emit("model-selection-updated", serde_json::json!({
        "group_id": if enabled { Some(group_id) } else { None },
        "timestamp": chrono::Utc::now().timestamp()
    })) {
        log::error!("Failed to emit model-selection-updated event: {}", e);
    }
